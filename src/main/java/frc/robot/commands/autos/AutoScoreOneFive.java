...
public class AutoScoreOneFive extends SequentialCommandGroup {
    ...
    public AutoScoreOneFive(DriveSubsystem drive, Arm arm, Grabber grabber, Lights lights) {
        addCommands(
            ...
            new FollowPathWithEvents(drive.followTrajectoryCommand(oneFive, true), oneFive.getMarkers(), eventMap)
                .withTimeout(2.56),
            new InstantCommand(() -> grabber.intake()),
            new WaitUntilCommand(()->grabber.seeGamePiece()),
            new InstantCommand(() -> grabber.spinMotors(GrabberCalibrations.HOLD_GAME_OBJECT_POWER))
        );
    }
